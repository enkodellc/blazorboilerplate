...
namespace BlazorBoilerplate.Shared.Helpers
{
    ...
    {
        public static readonly Regex StringInterpolationHelper = new("(?<=[\\ \"]{0,1}{)(.+?)(?=})(?:[\\ \\.\"]{0,2}|$)", RegexOptions.Compiled, TimeSpan.FromMilliseconds(100));

        //[a-zA-Z0-9.+= \\/-:]
        private static readonly Regex DirtyXmlPropertyParserRegex = new("(?<=<property key=\')(?<key>[a-zA-Z0-9\\- :]+?)(?:'>)(?<value>[^><]+?)(?=<\\/property>)", RegexOptions.Compiled, TimeSpan.FromMilliseconds(100));

...
//using Microsoft.Azure.KeyVault; DEPRECATED, using Azure.Security.KeyVault replacement libraries https://www.nuget.org/packages/Microsoft.Azure.KeyVault
//using Microsoft.Azure.Services.AppAuthentication; DEPRECATED, using Azure.Identity replacement library https://docs.microsoft.com/en-us/dotnet/api/overview/azure/service-to-service-authentication
                ...
                if (Convert.ToBoolean(Configuration["HostingOnAzure:RunsOnAzure"]) == true)
                {
                    if (Convert.ToBoolean(Configuration["HostingOnAzure:AzureKeyVault:UsingKeyVault"]) == true)
                    {
                        if (Convert.ToBoolean(Configuration["HostingOnAzure:AzureKeyVault:UseManagedAppIdentity"]) == true)
                        {
                            ...
                            // Azure Web App could be running multiple instances in multiple deployment slots. By default, the app stores encrypted cookies in local storage and
                            // separate instances can't access. We need to store in a central, protected place that can be accessed by all instances, and all instances must be
                            // able to unencrypt the content. We use Azure Blob Storage as the central store (persistence provider) and Azure Key Vault as the common encryption provider.
